...
class SplinePlanner(object):
    def __init__(self,dx_grid=None,dy_grid=None,acce_grid=None,dyaw_grid=None,max_steer=0.5, max_rvel=8, acce_bound=[-6, 4], vbound=[-10, 30],spline_order=3):
    ...
    def calc_trajs(self,x0,tf,xf):
        if x0.ndim==1:
            ...
            raise Exception("wrong dimension for x0")
    ...
    def gen_terminals(self,x0,tf):
        if x0.ndim==1:
            delta_x = np.array([
                np.array([x, y, v, yaw]) for x in self.dx_grid[2:3] for y in self.dy_grid for v in self.acce_grid[0::2]*tf for yaw in self.dyaw_grid[1:2]
            ])
            xy = np.concatenate((delta_x[:,0:1] + delta_x[:,2:3]/2*tf+x0[2:3]*tf,delta_x[:,1:2]),-1)
            rotated_xy = GeoUtils.batch_rotate_2D(xy, x0[3]) + x0[:2]
            return np.concatenate((rotated_xy,delta_x[:,2:]+x0[2:]),-1)
        elif x0.ndim==2:
            delta_x = np.array([
                np.array([x, y, v, yaw]) for x in self.dx_grid[2:3] for y in self.dy_grid for v in self.acce_grid[0::2]*tf for yaw in self.dyaw_grid[1:2]
            ])
            delta_x = np.tile(delta_x,[x0.shape[0],1,1])
            xy = np.concatenate((delta_x[:,:,0:1] + delta_x[:,:,2:3]/2*tf+x0[:,None,2:3]*tf,delta_x[:,:,1:2]),-1)
            rotated_xy = GeoUtils.batch_rotate_2D(xy, x0[:,3:4]) + x0[:,None,:2]

            return np.concatenate((rotated_xy,delta_x[:,:,2:]+x0[:,None,2:]),-1)
    def feasible_flag(self,traj):
        ...
        feas_flag = ((traj[...,2]>=self.vbound[0]) & (traj[...,2]<self.vbound[1]) &\
                    ...
                    (np.abs(traj[...,5]*traj[...,2])<=self.max_rvel) & (np.abs(traj[...,2])*self.max_steer>=np.abs(traj[...,5]))).all(1)
        return feas_flag
    def gen_trajs(self,x0,tf,dyn_filter=True):
        xf = self.gen_terminals(x0,tf)
        # x, y, v, a, yaw,r, t
        traj = jnp.asarray(self.calc_trajs(x0,tf,xf))
        if dyn_filter:
    ...
    def gen_traj_tree(self,x0,tf,n_layers,dyn_filter=True):
        trajs = list()
        nodes = [x0[None,:]]
        for i in range(n_layers):
            ...
            traj = self.calc_trajs(x0i,tf,xf)
            if dyn_filter:
                feas_flag = self.feasible_flag(traj)
                traj = traj[feas_flag]
                xf = xf[feas_flag]
...
if __name__=="__main__":
    ...
    tf = 5
    traj,xf = planner.gen_trajs(x0,tf)
    # x, y, v, a, yaw,r, t = traj
    msize = 12
    trajs,nodes = planner.gen_traj_tree(x0,tf,2)
